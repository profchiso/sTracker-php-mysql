...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($ref_no_err) && empty($docname_err) && empty($department_err) && empty($remark_err)){
		...
		exit(); 
			...
			$sql2 = "UPDATE paper SET current_handler = '$param_recipient' WHERE ref_no = '$param_ref_no'";
            ...
            <div class="wrap-login100">
				...
				while( $row = mysqli_fetch_array($result) ){
                    ...
                    <div class="form-group <?php echo (!empty($remark_err)) ? 'has-error' : ''; ?>">
                        <label>Remarks</label>
                        <textarea style="border-style:double" name="remark"
                            placeholder="Enter remarks. Cannot be empty." rows="5" cols="33%"></textarea>
